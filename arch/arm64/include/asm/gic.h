...
#define GICD_ISENABLER(n) \
	(0x0100 + (n) * 4) /* Interrupt Set-Enable Registers */
#define GICD_ICENABLER(n) \
	(0x0180 + (n) * 4) /* Interrupt Clear-Enable Registers */
#define GICD_ISPENDR(n) (0x0200 + (n) * 4) /* Interrupt Set-Pending Registers */
#define GICD_ICPENDR(n) \
	(0x0280 + (n) * 4) /* Interrupt Clear-Pending Registers */
#define GICD_ISACTIVER(n) \
	(0x0300 + (n) * 4) /* Interrupt Set-Active Registers */
#define GICD_ICACTIVER(n) \
	(0x0380 + (n) * 4) /* Interrupt Clear-Active Registers */
#define GICD_IPRIORITYR(n) (0x0400 + (n) * 4) /* Interrupt Priority Registers */
#define GICD_ITARGETSR(n) \
	(0x0800 + (n) * 4) /* Interrupt Processor Targets (v2 compat) */
#define GICD_ICFGR(n) (0x0c00 + (n) * 4) /* Interrupt Configuration Registers */
#define GICD_IGRPMODR(n) \
	(0x0d00 + (n) * 4) /* Interrupt Group Modifier Registers */
#define GICD_NSACR(n) \
	(0x0e00 + (n) * 4) /* Non-secure Access Control Registers */
...
#define GICD_IROUTER(n) \
	(0x6000 + (n) * 8) /* Interrupt Routing Registers (SPI ≥ 32) */
...
#define gicd_irouter_from_mpidr(mpidr)                                       \
	(GICD_IROUTER_AFF0((mpidr) >> 0) | GICD_IROUTER_AFF1((mpidr) >> 8) | \
	 GICD_IROUTER_AFF2((mpidr) >> 16) | GICD_IROUTER_AFF3((mpidr) >> 32))
