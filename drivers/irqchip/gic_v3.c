...
hv_status_t gic_v3_init(uintptr_t gicd_pa, uintptr_t gicr_pa,
                        uint32_t nr_cpus) {
...
hv_status_t gic_v3_enable_irq(uint32_t irq) {
  ...
  if (irq < GIC_SPI_BASE) {
    ...
    gic_write32(gic.gicd_base, GICD_ISENABLER(irq / 32), 1U << (irq % 32));
  }
  return HV_OK;
}
...
hv_status_t gic_v3_disable_irq(uint32_t irq) {
  ...
  if (irq < GIC_SPI_BASE) {
    ...
    gic_write32(gic.gicd_base, GICD_ICENABLER(irq / 32), 1U << (irq % 32));
  }
  gicd_wait_rwp(gic.gicd_base);
  return HV_OK;
}
...
hv_status_t gic_v3_route_irq(uint32_t irq, uint64_t mpidr) {
  ...
  gic_write64(gic.gicd_base, GICD_IROUTER(irq), gicd_irouter_from_mpidr(mpidr));
  return HV_OK;
}
...
uint32_t gic_v3_nr_irqs(void) { return gic.initialized ? gic.nr_irqs : 0; }
