 ...
 *
 * Returns 1 if windows [base1, base1+size1) and [base2, base2+size2) overlap, 0 otherwise.
 */
static int dma_windows_overlap(
	hv_u64 base1, hv_u64 size1,
	hv_u64 base2, hv_u64 size2
) {
...
hv_status_t hv_smmu_configure_dma_window(
	...
	device_dma_config[streamid].partition = smmu_state.streamid_to_partition[streamid];
