 ...
 * Global state.
 */
static hv_freertos_partition_t freertos_partitions[HV_MAX_FREERTOS_PARTITIONS] = {0};
static hv_task_t task_registry[HV_MAX_FREERTOS_TASKS_PER_PARTITION * HV_MAX_FREERTOS_PARTITIONS] = {0};
...
hv_status_t hv_freertos_create_partition(const hv_freertos_config_t *config, hv_u32 *partition_id) {
	...
	for (hv_u32 i = 0; i < HV_MAX_FREERTOS_PARTITIONS; i++) {
		if (!freertos_partitions[i].allocated) {
			...
			freertos_partitions[i].time_budget_us = config->time_budget_us;
			freertos_partitions[i].max_tasks = config->max_tasks;
			freertos_partitions[i].timer_frequency = config->timer_frequency;
...
hv_status_t hv_freertos_register_task(
	...
	if (freertos_partitions[partition].task_count >= freertos_partitions[partition].max_tasks) {
		return HV_ENOSPC;
	}

	/* Find available slot in registry. */
	for (hv_u32 i = 0; i < HV_MAX_FREERTOS_TASKS_PER_PARTITION * HV_MAX_FREERTOS_PARTITIONS; i++) {
...
hv_status_t hv_freertos_allocate_shared_region(
	...
	if (partition1 >= HV_MAX_FREERTOS_PARTITIONS || partition2 >= HV_MAX_FREERTOS_PARTITIONS) {
		return HV_EINVAL;
	}
	...
	if (!freertos_partitions[partition1].allocated || !freertos_partitions[partition2].allocated) {
		return HV_EPERM;
	}
	...
	for (hv_u32 i = 0; i < HV_MAX_SHARED_REGIONS; i++) {
		if (!shared_regions[i].enabled) {
			...
			shared_regions[i].start_addr = 0x80000000 + (i * size);  /* Assign virtual address */
...
hv_status_t hv_freertos_get_shared_address(hv_u32 region_id, hv_u32 partition, hv_u64 *address) {
	...
	if (partition != shared_regions[region_id].partition1 && partition != shared_regions[region_id].partition2) {
		return HV_EPERM;
	}
...
hv_status_t hv_freertos_destroy_partition(hv_u32 partition) {
	...
	for (hv_u32 i = 0; i < HV_MAX_FREERTOS_TASKS_PER_PARTITION * HV_MAX_FREERTOS_PARTITIONS; i++) {
		if (task_registry[i].allocated && task_registry[i].partition == partition) {
	...
	for (hv_u32 i = 0; i < HV_MAX_SHARED_REGIONS; i++) {
		if (shared_regions[i].enabled &&
			(shared_regions[i].partition1 == partition || shared_regions[i].partition2 == partition)) {
			memset(&shared_regions[i], 0, sizeof(shared_regions[0]));
			region_count--;
		}
	}

	/* Destroy partition. */
	memset(&freertos_partitions[partition], 0, sizeof(freertos_partitions[0]));
	partition_count--;
...
hv_status_t hv_freertos_get_stats(
	...
	if (task_count_ptr == NULL || context_switches == NULL || timer_ticks == NULL) {
		return HV_EINVAL;
	}
