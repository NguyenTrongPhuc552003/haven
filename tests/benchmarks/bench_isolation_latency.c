...
static hv_status_t b2_call(void)
{
	return hv_irq_is_owned_by(48, 1);
}
...
static hv_status_t b3_call(void)
{
	return hv_budget_consume(3, 1ULL);
}
...
static hv_status_t b6_call(void)
{
	return hv_iommu_check_group(6, 1);
}
...
int main(void)
{
	...
	results[idx++] = run_bench("stage2_partition_contains_pa", b1_setup,
				   NULL, b1_call, NULL);

	results[idx++] =
		run_bench("irq_is_owned_by", b2_setup, NULL, b2_call, NULL);

	results[idx++] =
		run_bench("budget_consume", b3_setup, NULL, b3_call, NULL);

	results[idx++] = run_bench("smmu_check_dma_access", b4_setup, NULL,
				   b4_call, NULL);

	results[idx++] = run_bench("timer_check_deadline", b5_setup, NULL,
				   b5_call, NULL);
