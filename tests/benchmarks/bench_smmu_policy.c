...
static void bench_setup(void)
{
	...
	hv_smmu_configure_dma_window(g_sid, DMA_IOVA_BASE, DMA_WINDOW_SZ,
				     DMA_PHYS_BASE, HV_DMA_RW);
}

/* -----------------------------------------------------------------------
 * Generic timing loop for hv_smmu_check_dma_access.
 * ----------------------------------------------------------------------- */
static bench_result_t run_check_case(const char *name, hv_u64 dma_addr,
...
static void write_json(bench_result_t *results, int n, const char *path)
{
	...
	fprintf(f, "{\n");
	fprintf(f, "  \"description\": \"Haven SMMU DMA policy enforcement "
		   "latency benchmark\",\n");
...
int main(void)
{
   ...
   */
	results[idx++] = run_check_case("smmu_check_window_boundary_low",
					DMA_IOVA_BASE, 0x1000ULL, HV_DMA_RO);

	/*
   * Case 3 - window high boundary: DMA at the last valid page.
   */
	results[idx++] =
		run_check_case("smmu_check_window_boundary_high",
   ...
   */
	results[idx++] =
		run_check_case("smmu_check_access_denied",
			       DMA_IOVA_BASE + DMA_WINDOW_SZ + 0x1000ULL,
			       0x1000ULL, HV_DMA_RO);
