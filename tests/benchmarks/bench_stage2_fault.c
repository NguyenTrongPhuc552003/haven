 ...
 * ----------------------------------------------------------------------- */
static bench_result_t run_case(const char *name,
                               hv_u32 partition_id,
                               hv_u64 pa, hv_u64 size)
{
    bench_result_t r;
    memset(&r, 0, sizeof(r));
    r.name  = name;
...
static void write_json(bench_result_t *results, int n, const char *path)
{
    ...
    fprintf(f, "{\n");
    fprintf(f,
            "  \"description\": \"Haven stage-2 containment check "
            "latency benchmark\",\n");
...
int main(void)
{
     ...
     */
    results[idx++] = run_case("stage2_contains_high_boundary",
                              BENCH_PARTITION_ID,
                              WINDOW_BASE + WINDOW_SIZE - 0x1000ULL,
                              0x1000ULL);
