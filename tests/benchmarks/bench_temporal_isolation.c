			    ...
			    int load_pct)
{
	...
	snprintf(buf, len, "rtos_response_%lldms_load_%dpct", period_ms,
		 load_pct);
}
...
static void write_json(temporal_result_t *results, int n, const char *path)
{
	...
	for (int i = 0; i < n; i++) {
		...
		fprintf(f,
			...
			results[i].load_pct, results[i].min_ns,
			results[i].mean_ns, results[i].max_ns, BENCH_ITERS,
			(i < n - 1) ? "," : "");
	}
...
int main(void)
{
	...
	for (int p = 0; p < n_periods; p++) {
		for (int l = 0; l < n_loads; l++) {
			char name[64];
			make_bench_name(name, sizeof(name), periods_ns[p],
					load_pcts[l]);
