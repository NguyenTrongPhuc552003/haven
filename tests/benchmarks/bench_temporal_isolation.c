 ...
 * ----------------------------------------------------------------------- */
static void make_bench_name(char *buf, size_t len, long long period_ns,
                            int load_pct) {
  long long period_ms = period_ns / 1000000LL;
  snprintf(buf, len, "rtos_response_%lldms_load_%dpct", period_ms, load_pct);
}
...
static void print_table(temporal_result_t *results, int n) {
  printf("\n%-45s  %9s  %9s  %8s  %8s  %8s\n", "benchmark", "period_ns",
         "load_pct", "min(ns)", "mean(ns)", "max(ns)");
  printf("%-45s  %9s  %9s  %8s  %8s  %8s\n",
         "---------------------------------------------", "---------",
         "---------", "--------", "--------", "--------");
  for (int i = 0; i < n; i++) {
    char name[64];
    make_bench_name(name, sizeof(name), results[i].period_ns,
                    results[i].load_pct);
    printf("%-45s  %9lld  %9d  %8lld  %8lld  %8lld\n", name,
           results[i].period_ns, results[i].load_pct, results[i].min_ns,
           results[i].mean_ns, results[i].max_ns);
  }
  printf("\n");
}
...
static void write_json(temporal_result_t *results, int n, const char *path) {
  ...
  fprintf(f, "{\n");
  fprintf(f, "  \"description\": \"Haven temporal isolation - "
  ...
  for (int i = 0; i < n; i++) {
    ...
    fprintf(f,
            ...
            "      \"iters\": %d\n"
            "    }%s\n",
            name, results[i].period_ns, results[i].budget_ns,
            results[i].load_pct, results[i].min_ns, results[i].mean_ns,
            results[i].max_ns, BENCH_ITERS, (i < n - 1) ? "," : "");
  }
