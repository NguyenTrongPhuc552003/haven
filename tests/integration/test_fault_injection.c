...
static struct hv_partition_mem p1_mem = {.partition_id = 1,
					 .regions = &p1_reg,
					 .region_count = 1};
...
static struct hv_partition_mem p2_mem = {.partition_id = 2,
					 .regions = &p2_reg,
					 .region_count = 1};

static struct hv_irq_route p1_irq = {.irq_id = 64,
				     .owner_partition_id = 1,
				     .target_cpu = 0};
static struct hv_irq_route p2_irq = {.irq_id = 65,
				     .owner_partition_id = 2,
				     .target_cpu = 1};

static struct hv_budget p1_bgt = {.partition_id = 1,
				  .period_ns = 1000000ULL,
				  .budget_ns = 500000ULL};
static struct hv_budget p2_bgt = {.partition_id = 2,
				  .period_ns = 1000000ULL,
				  .budget_ns = 200000ULL};
...
static void setup(void)
{
	...
	assert(hv_smmu_configure_dma_window(sid1, 0x40000000, 0x02000000,
					    0x40000000, HV_DMA_RW) == HV_OK);
	assert(hv_smmu_allocate_streamid(2, &sid2) == HV_OK);
	assert(hv_smmu_configure_dma_window(sid2, 0x50000000, 0x00800000,
					    0x50000000, HV_DMA_RO) == HV_OK);
...
static void fault_f4_smmu_escape(void)
{
	...
	assert(hv_smmu_allocate_streamid(2, &sid2) == HV_OK);
	CHECK(hv_smmu_configure_dma_window(sid2, 0x50000000, 0x00800000,
					   0x40000000, HV_DMA_RO) == HV_EPERM,
	      "F4: deny  - P2 stream cannot target P1's PA");
	/* P1's DMA window is intact. */
	CHECK(hv_smmu_check_dma_access(sid1, 0x41000000, 0x1000, HV_DMA_RW) ==
		      HV_OK,
	      "F4: intact - P1 DMA access still works");
	/* Restore P2's stream to a valid config for subsequent tests. */
	assert(hv_smmu_configure_dma_window(sid2, 0x50000000, 0x00800000,
					    0x50000000, HV_DMA_RO) == HV_OK);
	CHECK(hv_smmu_check_dma_access(sid2, 0x50100000, 0x1000, HV_DMA_RO) ==
		      HV_OK,
...
static void fault_f5_timer_hijack(void)
{
   ...
   */
	...
	CHECK(expired == 0,
	      "F5: intact - P1 deadline unmodified by P2 operations");
}
