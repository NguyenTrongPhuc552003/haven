...
static void run_negative_isolation_flow(void)
{
	...
	assert(hv_stage2_unmap_partition(conflicting_pa_cfg.partition_id) ==
	       HV_OK);
	...
	assert(hv_irq_is_owned_by(owner_route.irq_id,
				  foreign_route.owner_partition_id) ==
	       HV_EPERM);
	assert(hv_irq_revoke(owner_route.irq_id,
			     foreign_route.owner_partition_id) == HV_EPERM);
	assert(hv_irq_revoke(owner_route.irq_id,
			     owner_route.owner_partition_id) == HV_OK);
