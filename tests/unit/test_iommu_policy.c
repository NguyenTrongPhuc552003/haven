...
static void test_iommu_assign_group(void)
{
	...
	assert(s == HV_EPERM);
	TEST_PASS(
		"iommu_assign_group: denies reassignment to different partition");
	...
	assert(s == HV_EINVAL);
	TEST_PASS(
		"iommu_assign_group: rejects partition >= HV_MAX_IOMMU_PARTITIONS");
}
...
static void test_iommu_lifecycle(void)
{
	...
	TEST_PASS(
		"iommu_lifecycle: assign → deny-foreign → release → reassign");
}
