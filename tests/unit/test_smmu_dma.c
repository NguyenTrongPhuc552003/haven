...
static void setup_partition_memory(hv_u32 partition, hv_u64 pa_base,
				   hv_u64 size)
{
...
{
	hv_smmu_init();
	setup_partition_memory(1, 0xc0000000, 0x10000000);

	hv_u16 sid1, sid2;
	hv_status_t status;

	/* Allocate first device. */
	status = hv_smmu_allocate_streamid(1, &sid1);
	assert(status == HV_OK);
	assert(sid1 == 0);
...
static void test_smmu_configure_dma_window(void)
{
	...
	/* Allocate device. */
	status = hv_smmu_allocate_streamid(1, &sid);
	assert(status == HV_OK);

	/* Configure basic DMA window. */
	status = hv_smmu_configure_dma_window(
		sid, 0x80000000, /* DMA base (IOVA) */
	...
	/* Negative: Reconfigure without revoke. */
	status = hv_smmu_configure_dma_window(sid, 0x90000000, 0x10000000,
					      0xc0000000, HV_DMA_RW);
	assert(status == HV_EPERM);
	TEST_PASS(
		"smmu_configure_dma_window: rejects reconfigure without revoke");

	/* Negative: Invalid StreamID. */
	status = hv_smmu_configure_dma_window(HV_MAX_SMMU_DEVICES + 1,
	...
	status = hv_smmu_configure_dma_window(10, 0x80000000, 0x10000000,
					      0xc0000000, HV_DMA_RW);
	assert(status == HV_EPERM);
	TEST_PASS("smmu_configure_dma_window: rejects unallocated streamid");

	/* Allocate a fresh device for parameter-validation checks. */
	status = hv_smmu_allocate_streamid(1, &sid_fresh);
	assert(status == HV_OK);

	/* Negative: Misaligned DMA address. */
	status = hv_smmu_configure_dma_window(sid_fresh,
	...
	status = hv_smmu_configure_dma_window(sid_fresh, 0x80000000, 0x10000000,
					      0xc0000001, /* Not 4K aligned */
					      HV_DMA_RW);
	assert(status == HV_EINVAL);
	TEST_PASS(
		"smmu_configure_dma_window: rejects misaligned physical address");

	/* Negative: Size not power-of-2. */
	status = hv_smmu_configure_dma_window(sid_fresh, 0x80000000,
					      0x10000001, /* Not power-of-2 */
					      0xc0000000, HV_DMA_RW);
	assert(status == HV_EINVAL);
	TEST_PASS("smmu_configure_dma_window: rejects non-power-of-2 size");

	/* Negative: Invalid access flag. */
	status = hv_smmu_configure_dma_window(sid_fresh, 0x80000000, 0x10000000,
					      0xc0000000, (hv_dma_access_t)0);
	assert(status == HV_EINVAL);
	TEST_PASS("smmu_configure_dma_window: rejects invalid access flag");

	/* Negative: DMA window outside owning partition memory. */
	status = hv_smmu_configure_dma_window(sid_fresh, 0x80000000, 0x10000000,
					      0xd0000000, HV_DMA_RW);
	assert(status == HV_EPERM);
	TEST_PASS(
		"smmu_configure_dma_window: rejects window outside partition memory");

	/* Positive: Revoke then reallocate and configure succeeds. */
	...
	assert(status == HV_OK);
	TEST_PASS(
		"smmu_configure_dma_window: reconfigure after revoke succeeds");
}
...
{
	hv_smmu_init();
	setup_partition_memory(1, 0xc0000000, 0x20000000);
	...
	status = hv_smmu_configure_dma_window(
		sid1, 0x80000000, 0x10000000, /* 0x80000000 - 0x90000000 */
		0xc0000000, HV_DMA_RW);
	assert(status == HV_OK);
	...
	status = hv_smmu_configure_dma_window(
		sid2, 0x88000000, /* Overlaps with first window */
		0x08000000, 0xd0000000, HV_DMA_RW);
	...
	status = hv_smmu_configure_dma_window(
		sid2, 0x90000000, /* After first window */
		0x10000000, 0xd0000000, HV_DMA_RW);
	assert(status == HV_OK);
...
static void test_smmu_revoke_dma_access(void)
{
	...
	status = hv_smmu_allocate_streamid(1, &sid);
	assert(status == HV_OK);
	status = hv_smmu_configure_dma_window(sid, 0x80000000, 0x10000000,
					      0xc0000000, HV_DMA_RW);
	assert(status == HV_OK);
...
static void test_smmu_check_dma_access(void)
{
	...
	status = hv_smmu_allocate_streamid(1, &sid_rw);
	assert(status == HV_OK);
	status = hv_smmu_configure_dma_window(sid_rw, 0x80000000, 0x10000000,
					      0xc0000000, HV_DMA_RW);
	assert(status == HV_OK);

	/* Setup read-only device. */
	status = hv_smmu_allocate_streamid(1, &sid_ro);
	assert(status == HV_OK);
	status = hv_smmu_configure_dma_window(sid_ro, 0x90000000, 0x10000000,
					      0xd0000000, HV_DMA_RO);
	assert(status == HV_OK);

	/* RW device can read. */
	status =
		hv_smmu_check_dma_access(sid_rw, 0x80000000, 0x1000, HV_DMA_RO);
	assert(status == HV_OK);
	TEST_PASS("smmu_check_dma_access: RW device permits read");

	/* RW device can write. */
	status =
		hv_smmu_check_dma_access(sid_rw, 0x80000000, 0x1000, HV_DMA_WO);
	assert(status == HV_OK);
	TEST_PASS("smmu_check_dma_access: RW device permits write");

	/* RO device can read. */
	status =
		hv_smmu_check_dma_access(sid_ro, 0x90000000, 0x1000, HV_DMA_RO);
	assert(status == HV_OK);
	TEST_PASS("smmu_check_dma_access: RO device permits read");

	/* Negative: RO device cannot write. */
	status =
		hv_smmu_check_dma_access(sid_ro, 0x90000000, 0x1000, HV_DMA_WO);
	assert(status == HV_EPERM);
	TEST_PASS("smmu_check_dma_access: RO device denies write");

	/* Negative: Access outside DMA window. */
	status =
		hv_smmu_check_dma_access(sid_rw, 0xa0000000, 0x1000, HV_DMA_RW);
	assert(status == HV_EPERM);
...
static void test_smmu_reset_partition(void)
{
	...
	/* Configure both. */
	status = hv_smmu_configure_dma_window(sid1, 0x80000000, 0x10000000,
					      0xc0000000, HV_DMA_RW);
	assert(status == HV_OK);
	status = hv_smmu_configure_dma_window(sid2, 0x90000000, 0x10000000,
					      0xd0000000, HV_DMA_RW);
	assert(status == HV_OK);

	/* Reset partition 1. */
	status = hv_smmu_reset_partition(1);
	assert(status == HV_OK);
	...
	/* Positive: revoked StreamIDs can be reused. */
	status = hv_smmu_allocate_streamid(1, &sid1);
	assert(status == HV_OK);
	assert(sid1 == 0);
	TEST_PASS("smmu_reset_partition: streamid can be reused after reset");
}
