...
static void test_smmu_configure_dma_window(void) {
  ...
  status =
      hv_smmu_configure_dma_window(sid, 0x80000000, /* DMA base (IOVA) */
  ...
  status = hv_smmu_configure_dma_window(sid, 0x90000000, 0x10000000, 0xc0000000,
                                        HV_DMA_RW);
  assert(status == HV_EPERM);
  TEST_PASS("smmu_configure_dma_window: rejects reconfigure without revoke");

  /* Negative: Invalid StreamID. */
  status = hv_smmu_configure_dma_window(HV_MAX_SMMU_DEVICES + 1, 0x80000000,
                                        0x10000000, 0xc0000000, HV_DMA_RW);
  assert(status == HV_EINVAL);
  TEST_PASS("smmu_configure_dma_window: rejects invalid streamid");

  /* Negative: Unallocated StreamID. */
  status = hv_smmu_configure_dma_window(10, 0x80000000, 0x10000000, 0xc0000000,
                                        HV_DMA_RW);
  assert(status == HV_EPERM);
  ...
  status =
      hv_smmu_configure_dma_window(sid_fresh, 0x80000001, /* Not 4K aligned */
                                   0x10000000, 0xc0000000, HV_DMA_RW);
  assert(status == HV_EINVAL);
  ...
  status = hv_smmu_configure_dma_window(sid, 0x90000000, 0x10000000, 0xc0000000,
                                        HV_DMA_RW);
  assert(status == HV_OK);
  TEST_PASS("smmu_configure_dma_window: reconfigure after revoke succeeds");
}
...
static void test_smmu_window_overlap_detection(void) {
  ...
  status =
      hv_smmu_configure_dma_window(sid2, 0x90000000, /* After first window */
...
static void test_smmu_revoke_dma_access(void) {
  ...
  status = hv_smmu_allocate_streamid(1, &sid);
  assert(status == HV_OK);
  status = hv_smmu_configure_dma_window(sid, 0x80000000, 0x10000000, 0xc0000000,
                                        HV_DMA_RW);
  assert(status == HV_OK);
