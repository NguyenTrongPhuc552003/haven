...
`cmake --preset arm64-qemu` build of the Haven hypervisor.
...
2. **Build invocation** - runs:
   ```bash
   CROSS_COMPILE=<detected-prefix> cmake --preset arm64-qemu
...
| `--jobs`      | `nproc`       | Parallel cmake build jobs                                      |
...
| CMake                           | 3.22+           |                               |
| Ninja                           | 1.10+           | CMake build backend           |
