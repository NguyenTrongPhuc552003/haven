...
| Haven build       | `make ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- all`     |
...
The following benchmarks ship in `tests/benchmarks/` and produce JSON output
in `build/benchmarks/`:

| Benchmark file                       | Output JSON                      | What it measures                              |
| ------------------------------------ | -------------------------------- | --------------------------------------------- |
| `bench_isolation_latency.c`          | `isolation-latency.json`         | 6 enforcement hot-path per-call latencies     |
| `bench_stage2_fault.c`               | `stage2-fault.json`              | Stage-2 containment check, 4 boundary cases   |
| `bench_smmu_policy.c`                | `smmu-policy.json`               | SMMU DMA policy, 5 cases + StreamID alloc     |
| `bench_temporal_isolation.c`         | `temporal-isolation.json`        | RTOS response time, 3 periods × 5 load levels |

Each benchmark runs 100 000 iterations (10 000 for temporal) with 1 000 warm-up
iterations discarded.  Reported values: min / mean / max in nanoseconds.

Analysis scripts in `tools/analysis/`:
- `latency_analyzer.py` — parses JSON, computes p99/p99.9/max per operation.
- `jitter_plot.py` — Matplotlib histograms; overlays QEMU vs i.MX95 data.
- `evidence_report.py` — HTML evidence report from all JSON results.
...
Nightly CI (`evidence-generation` job in `.github/workflows/nightly.yml`)
archives the full bundle with 90-day retention.
